//! 测试专用合成 SOFA fixture 生成器。
//!
//! 按切片门禁要求，本 crate **不捆绑任何真实 SOFA/HRTF 文件**（资产合规
//! 审计未完成）。这里的生成器参照 HDF5 规范中 `sofar` 解析器实际支持的
//! 最小子集，程序化构造 SimpleFreeFieldHRIR 数据（单位脉冲、对称左右耳
//! HRIR、4 个水平方向测量），直接在内存中拼出完整字节流，供落盘或直接
//! 走 `load_sofa_bytes` 使用。
//!
...
//!
//! 仅覆盖合成 happy-path 与损坏注入所需的格式面；真实 SOFA 文件的兼容性
//! 验收仍然依赖用户自备资产（见 `sofa.rs` 中被 ignore 的资产测试）。

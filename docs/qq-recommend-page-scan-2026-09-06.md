...
WaveForge 适配结论：猜你喜欢和每日30首使用同等尺寸；其余四项也作为真实入口卡片展示，不能用公共歌单替代。卡片中的“30首连续推荐”属于实现描述，应删除。
...
WaveForge 适配结论：应建独立 `personal-feed` 模型和区块，不能把它压成 `Song[]`，也不能伪装成推荐歌单。feed 游标必须与猜你喜欢连续播放的 batch 分开。

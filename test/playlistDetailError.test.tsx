...
describe('PlaylistDetailPanel errors', () => {
  it('shows a retriable error instead of an empty playlist state', () => {
    ...
    render(
      <PlaylistDetailPanel
        show
        playlist={{ id: 'pl.1', name: 'Test Playlist', coverImgUrl: '', trackCount: 10, platform: 'qq' }}
        songs={[]}
        loading={false}
        error="QQ 音乐登录状态无效，请重新登录"
        onRetry={onRetry}
        onClose={vi.fn()}
        onSongSelect={vi.fn()}
        currentPlatform="qq"
    ...
    expect(screen.getByRole('alert').textContent).toContain('QQ 音乐登录状态无效')

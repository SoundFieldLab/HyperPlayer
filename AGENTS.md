...
- **启动页（splash）流水线 —— 预渲染视频版**：`new-splash/splash-webm.webm`（设计侧录制的 VP9/WebM，1401×900、3.2s）→ `npm run build:splash`（`scripts/build-splash.mjs`）→ `desktop/splash.html` + `desktop/splash-baked.webm`。
  - **为什么用预渲染**：启动页原本实时跑 18 层大半径模糊 + JS 流体/锁体动画，启动瞬间与主窗口加载抢 CPU，导致进场动画出帧滞后、观感发卡。预渲染把开销前移到构建期，运行时只解码（走 GPU，几乎不占 CPU），动画必然流畅。
  ...
  - **时序契约（`desktop/main.cjs` 依赖，勿改）**：双向信号。① 主进程在启动页窗口真正 `show()` 后发 `splash:start` 放行，页面收到**才开始播放**——页面刻意不用 autoplay：视频若在页面加载即起跑，而窗口 ~500ms 后才显示，用户会错过开头（logo 弹出一半）；② 视频 `ended`（或播放失败）时页面调 `window.splashBridge.entranceDone()`（桥在 `desktop/splash-preload.cjs`）→ 主进程 `showMainWindowWhenReady` 满足「启动页已显示 + 最短可见 + 动画播完」三条件才切主窗口（切换兜底锚定**启动页显示时刻**，勿锚回主窗口就绪时刻）。这样动画起点与用户所见对齐、且「动画完整播完」才切走。
  - **舞台几何**：视频按**自身比例（1401×900，非 16:9）以 cover 铺满窗口**——窗口比例与视频一致（默认 1400×900）时 1:1 原样呈现，其它比例只裁掉边缘抽象渐变，logo 组位于画面中部永不被裁。**勿把舞台写死 16:9**：写死会在非 16:9 窗口（用户窗口尺寸有记忆）上把视频放大 1.3 倍并四边裁切，构图明显失真（2026-09-13 用户实测发现）。

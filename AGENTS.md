...
- `desktop/` — Electron main process, **CommonJS**（`main.cjs`, `preload.cjs`, `desktop-lyrics-preload.cjs`, `desktop-player-preload.cjs`, `taskbar-widget-preload.cjs`, `config-manager.cjs`, `window-state.cjs`, `user-data-profile.cjs`, `trusted-ipc.cjs`, `update-manager.cjs`, `update-applier.cjs`, `vmp-status.cjs`, `audio-download.cjs`, `apple-url-policy.cjs`, `chroma-*.cjs`, `signalrgb-*.cjs`, `razer-device-discovery.cjs`, `taskbar-widget-polling.cjs`, `splash.html`, `taskbar-widget.html`）。`splash.html` 是**生成物**（由 `scripts/build-splash.mjs` 从 `new-splash/hyperplayer-splash/` 产出，勿手改）；它是**离线 file:// 页面**：字体与 logo 全部内联为 data URI，CSP 禁止一切外部资源。Not covered by `tsc --noEmit`。
- **启动页（splash）流水线**：`new-splash/hyperplayer-splash/`（设计侧产物：`index.html` 自包含成品 + `build.py` + `assets/`）→ `npm run build:splash`（`scripts/build-splash.mjs` 适配器）→ `desktop/splash.html`。适配器做两件事：① 把设计稿固定 1920×1080 画布的 px 几何统一换算为 **cqw**，舞台恒 16:9 并按 **cover** 铺满窗口（任意窗口尺寸/比例下构图与设计稿等比一致，logo 恒居中永不被裁）；② 去掉 `Glass Sheet` 那一层**全屏** `backdrop-filter`（实测单层占 21% 渲染开销；其背后已是重度模糊光斑，去掉前后平均色差仅 0.55/255，肉眼不可辨）。运动学（速度/时长/缓动/颜色/图层参数）原样保留。**勿再削减光斑自身的 `filter: blur`**——那是柔边观感的来源，去掉会退化成硬边圆角矩形（视觉不合格，已试过）。

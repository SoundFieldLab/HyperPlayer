...
import { CaretDown, CaretUp, CloudArrowDown, DotsThree, Heart, ListPlus, Play, Queue, SpinnerGap, Trash, Waveform, X } from "@phosphor-icons/react";
...
export function TrackTable({ tracks, compact = false }: { tracks: TrackDto[]; compact?: boolean }) {
  ...
  const commands = (track: TrackDto) => {
    ...
    return [
      { id: "play", label: targets.length === 1 ? "播放" : `播放所选 ${targets.length} 首`, icon: Play, run: () => void playTrack(targets[0]) },
      ...
      { id: "favorite", label: "喜欢", icon: Heart, run: () => { setUndo({ ids: [], label: `已喜欢 ${targets.length} 首歌曲` }); setMenu(null); } },
      { id: "remove", label: "从当前列表移除", icon: X, run: () => removeWithUndo(targets) },
  ...
  return <div className={`advanced-track-wrap ${compact ? "compact" : ""}`}>
    ...
    <div className="track-table" role="grid" aria-label="曲目列表" onKeyDown={(event) => {
      ...
      {ordered.map((track, index) => {
        ...
        return <div role="row" data-track-id={track.id} aria-selected={selectedTrackIds.includes(track.id)} tabIndex={0} key={track.id} className={`track-row ${selectedTrackIds.includes(track.id) ? "selected" : ""} ${current ? "current" : ""}`} style={{ gridTemplateColumns: template }} onClick={(event) => select(track, event)} onDoubleClick={() => void playTrack(track)} onContextMenu={(event) => { event.preventDefault(); if (!selectedTrackIds.includes(track.id)) select(track, event); setMenu({ x: event.clientX, y: event.clientY, track }); }} onKeyDown={(event) => { if (event.key === "Enter") void playTrack(track); }}>

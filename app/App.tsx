...
function MainApp(): React.JSX.Element {
  ...
  return <div className={`app-shell material-${state.settings?.material} ${state.settings?.reduceTransparency ? "reduce-transparency" : ""} ${state.settings?.reduceMotion ? "reduce-motion" : ""}`} data-theme={theme}><SidebarNav/><div className="workspace"><Titlebar/><main className="content" tabIndex={-1}><CurrentView/></main></div><PlayerDock/><AnimatePresence>{state.expandedPlayer && <ExpandedPlayer/>}{state.searchOpen && <CommandPalette/>}{state.overlay === "queue" && !state.expandedPlayer && <QueuePanel/>}{state.queueFloating && <QueuePanel floating/>}{state.miniOpen && <MiniPlayer/>}{state.desktopLyricsOpen && <DesktopLyrics/>}</AnimatePresence><UtilityLauncher/><ToastRegion/><CloseConfirmDialog/></div>;
}

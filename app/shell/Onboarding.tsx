...
export function Onboarding(): React.JSX.Element {
  ...
  return <main className="onboarding">
    ...
    <AnimatePresence mode="wait"><motion.section key={step} className="onboarding-panel" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
      ...
      {step === 3 && <><p className="eyebrow">音频输出</p><h1>音频输出</h1><p className="lead">DSP 规格尚未接入，初始保持旁路。</p><div className="notice"><Info/>播放器将在首次播放时验证 Windows 默认音频设备。</div><div className="notice"><SlidersHorizontal/> 音效当前不可用，将在真实 DSP 规格接入后开放。</div></>}
      {step === 4 && <><p className="eyebrow">设置完成</p><h1>开始聆听</h1><p className="lead">将进入网易云公开首页。队列会恢复，但启动时不会自动出声。</p><div className="summary-list"><span><Sun/>明亮主题</span><span><User/>匿名浏览</span><span><FolderOpen/>本地目录可稍后添加</span><span><SpeakerHigh/>系统默认输出</span></div></>}

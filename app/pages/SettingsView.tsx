...
export function SettingsView(): React.JSX.Element | null {
  ...
  else if (category === "audio") content = <><div className="dsp-status-summary"><SlidersHorizontal /><div><h3>DSP 核心接入中</h3><p>Rust 已接入 M/S、参数均衡、动态压缩与低频增强；完整参数桥、预设和分析器仍在实施。</p></div><span>4 个阶段</span></div><ReadonlyRow title="处理权威" detail="所有实际播放处理均在 Rust 音频线程完成" value="Rust Engine" /><ReadonlyRow title="安全状态" detail="处理器故障时恢复原始 PCM 并锁定旁路" value="Fail-safe" /><ReadonlyRow title="输出设备" detail="播放时由 Rust 音频引擎验证 Windows 默认设备" value="自动选择 · 只读" /></>;

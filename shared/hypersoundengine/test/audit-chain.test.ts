...
/** 全效果关闭的基础参数（旁路基座） */
function allOffParams(): HyperSoundEngineParams {
...
describe('② 逐效果 enabled=false 真正旁路', () => {
  it('eq(无档案)/deesser/compressor/nightMode/reverb/bass/loudnessComp/ieq/surround3d/limiter/loudNorm 全部直通（diff ≤1e-9）', () => {
    const cases: Array<[string, (p: HyperSoundEngineParams) => void]> = [
...
describe('⑥ 场景与组合链路', () => {
  it('全部 12 个场景：128 样本块长跑 2s 无 NaN、输出有界（≤3）', () => {
    // HyperSoundEngine 侧 vitest 4 对同步长测试也强制默认 5s 超时（本机实测 ~8s），显式放宽
    const B = 128
    const l = sine(B, 330, 0.5, FS)
    const r = zeros(B)

...
import type { ScenePreset, HyperSoundEngineParams } from '../src/types'
...
function expectValidParams(p: HyperSoundEngineParams, id: string): void {
  // EQ
...
describe('ScenePresets 结构', () => {
  it('SCENE_IDS 长度 12 且唯一', () => {
    expect(SCENE_IDS.length).toBe(12)
    expect(new Set(SCENE_IDS).size).toBe(12)
    expect(Array.from(SCENE_IDS)).toEqual(EXPECTED_IDS)
  })

  it('SCENE_PRESETS 长度 12，id 与 SCENE_IDS 一一对应，builtin=true', () => {
    expect(SCENE_PRESETS.length).toBe(12)

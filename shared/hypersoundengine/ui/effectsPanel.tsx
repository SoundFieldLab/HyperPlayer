...
export type EffectUiKey =
  ...
  | 'delay' | 'chorus' | 'flanger' | 'phaser' | 'tremolo' | 'modulation'
...
export const EFFECT_META: Record<EffectUiKey, { name: string; desc: string; intro: string; icon: LucideIcon; row?: boolean }> = {
  ...
  delay: {
    name: '延迟', desc: '回声 / 空间纵深', intro: '环形延迟线 + 反馈 + 干湿混合，用于回声、空间纵深与节奏型重复。', icon: Timer,
  },
  chorus: {
    name: '合唱', desc: '厚度 / 空间感', intro: '多路 LFO 调制分数延迟，营造厚度与空间感，适合吉他、人声、合成器铺底。', icon: Waves,
  },
  flanger: {
    name: '镶边', desc: '喷气扫频', intro: '短延迟 + LFO 调制 + 反馈，产生喷气式扫频梳状滤波，标志性金属扫频效果。', icon: Wind,
  },
  phaser: {
    name: '移相', desc: '相位扫频', intro: '多级全通滤波器 + LFO 调制中心频率 + 反馈，产生柔和的相位扫频，比镶边更平滑。', icon: Zap,
  },
  tremolo: {
    name: '颤音', desc: '音量起伏', intro: 'LFO 调制信号幅度，产生周期性音量起伏，常用于吉他复古颤音与氛围铺底。', icon: Volume2,
  },
  modulation: {
    name: '参数调制矩阵', desc: 'LFO / 包络 → 参数', intro: 'LFO 与包络跟随器作为调制源，按路由叠加到主增益或立体声宽度，实现呼吸、摆动与动态控制。', icon: Radio, row: true,
  },
...
/** 网格卡片顺序（响度类两卡以独立行呈现） */
const GRID_KEYS: EffectUiKey[] = ['reverb', 'surround3d', 'bassEnhancer', 'compressor', 'nightMode', 'deesser', 'ieq', 'limiter', 'pitch', 'stereoWidth', 'delay', 'chorus', 'flanger', 'phaser', 'tremolo']
const ROW_KEYS: EffectUiKey[] = ['loudnessCompensation', 'loudnessNormalization']
const MOD_KEY: EffectUiKey = 'modulation'
...
export function effectEnabled(p: HyperSoundEngineParams, key: EffectUiKey): boolean {
  switch (key) {
    ...
    case 'delay': return p.modEffects.delay.enabled
    case 'chorus': return p.modEffects.chorus.enabled
    case 'flanger': return p.modEffects.flanger.enabled
    case 'phaser': return p.modEffects.phaser.enabled
    case 'tremolo': return p.modEffects.tremolo.enabled
    case 'modulation': return p.modulation.enabled && p.modulation.routes.length > 0
...
export function patchEffectEnabled(patch: (partial: DeepPartial<HyperSoundEngineParams>) => void, p: HyperSoundEngineParams, key: EffectUiKey, on: boolean): void {
  switch (key) {
    ...
    case 'delay': patch({ modEffects: { ...p.modEffects, delay: { ...p.modEffects.delay, enabled: on } } }); return
    case 'chorus': patch({ modEffects: { ...p.modEffects, chorus: { ...p.modEffects.chorus, enabled: on } } }); return
    case 'flanger': patch({ modEffects: { ...p.modEffects, flanger: { ...p.modEffects.flanger, enabled: on } } }); return
    case 'phaser': patch({ modEffects: { ...p.modEffects, phaser: { ...p.modEffects.phaser, enabled: on } } }); return
    case 'tremolo': patch({ modEffects: { ...p.modEffects, tremolo: { ...p.modEffects.tremolo, enabled: on } } }); return
    case 'modulation': patch({ modulation: { ...p.modulation, enabled: on, routes: on && p.modulation.routes.length === 0 ? [{ source: 'lfo', target: 'masterGain', amount: 0.3, offset: 0 }] : p.modulation.routes } }); return
...
export function EffectsPanel({ controller, bridge, theme, onOpenEffect }: EffectsPanelProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* 参数调制矩阵（独立卡片） */}
      <SectionTitle icon={<Radio className="w-4 h-4" style={{ color: theme.accentColor }} />} theme={theme}>参数调制</SectionTitle>
      {renderRowCard(MOD_KEY)}

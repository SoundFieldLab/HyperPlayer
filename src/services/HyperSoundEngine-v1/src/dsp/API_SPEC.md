...
> 本文件是 HyperSoundEngine-v1 各 DSP 模块的**唯一实现契约**。所有子代理严格按此签名实现，
...
9. 边界（严格遵守）：
   ...
   - **禁止读取**：`docs/`（逆向分析文档，另一个对话的产物）、`decompiled/`、`business-code/`、`apktool-out/`、`reference/`、`HyperPlayer/src/services/audio-effects-v3/`（另一个对话正在做的工作）；
   - **禁止写入**：除自己负责的文件外的一切文件；HyperPlayer/、research/、docs/ 一律只读；

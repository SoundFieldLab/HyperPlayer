...
export default function ScenesPage({ bridge, controller, theme, onOpenEffect }: ScenesPageProps) {
  ...
  const handleExportPublishSeed = async () => {
    try {
      ...
      a.click()
      URL.revokeObjectURL(url)
      notify(res?.error ? `自动写回失败（${res.error}），已改为下载 —— 请手动替换 src/services/HyperSoundEngine-v1/src/engine/builtinSceneSeed.ts 并提交` : '当前环境不支持直接写盘，已改为下载 —— 请手动替换 src/services/HyperSoundEngine-v1/src/engine/builtinSceneSeed.ts 并提交', 'info')
    } catch {

# HyperPlayer v3 调音室 UI —— 融合指南（UI_GUIDE）

> 配套 `docs/FUSION_GUIDE.md`（引擎融合手册）。本文档讲 **UI 部分**：
> `HyperSoundEngine-v1/ui/` 的现状（HSE 风格）、接线 `V3MixingStudio`、接通系统音量与听力测试播放。
...
1. 拷贝目录：`HyperSoundEngine-v1/ui/` → `HyperPlayer/src/services/HyperSoundEngine-v1/ui/`；
2. 调整 import：ui/ 内引用引擎的路径为 `../src/types` 等相对路径，拷贝后层级不变（`src/services/HyperSoundEngine-v1/` 下 ui/ 与 src/ 同级），无需改动。

> 若 HyperPlayer 侧已有 `src/services/HyperSoundEngine-v1/`（FUSION_GUIDE 步骤 1 的引擎目录），
...
const loadMixingStudioV3 = () => import('./services/HyperSoundEngine-v1/ui')
const LazyMixingStudioV3 = lazy(loadMixingStudioV3)

// 2) 切换 v3 引擎后，用 EngineV3Host 的 engine 建桥：
import { createV3UiBridge } from './services/HyperSoundEngine-v1/ui'

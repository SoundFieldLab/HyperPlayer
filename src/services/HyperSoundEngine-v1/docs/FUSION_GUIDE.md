...
├── test/ + ui/uiSmoke.test.tsx        # 324 用例（融合时迁入 HyperPlayer/test/ 或加 include）
...
（可复用 HyperPlayer 现有 `exportToWav` 的编码部分，替换效果链执行为 v3 引擎）。
...
| 2 | HyperPlayer 回归 | `npm run lint` 0 错误；`npm test` 既有用例不回归 |
...
- 空间节点是 v3 处理节点**之后**的独立 AudioWorklet 节点（处理器注册名 `hyperplayer-spatial`，
...
- **EngineV3 零改动约束**：空间化不进 `EngineV3.ts` 14 级链、不在 `V3EngineParams` 加字段、
  ...
  持久化于 localStorage `hyperplayer:spatial-params`。
...
`src/spatial/`（相对 `src/services/HyperSoundEngine-v1/`）：
...
| `src/spatial/SpatialProcessor.ts` | AudioWorklet 处理器（`hyperplayer-spatial`：双耳渲染 / 多声道输入 `processMulti` / 输出 >2 声道物理映射 / stats 回传） |
...
| `src/spatial/hrtfStore.ts` | HRTF 数据集 IndexedDB 持久化（db `hyperplayer-hrtf` / store `datasets`，手写 Promise 封装） |
| `src/spatial/persistence.ts` | 参数持久化（`hyperplayer:spatial-params`，400ms 防抖 + 深合并容错，存储可注入） |
...
| 键 | 内容 | 说明 |
|---|---|---|
| `hyperplayer:spatial-params` | SpatialParams 快照 | 400ms 防抖 + 与默认值深合并（数组整段替换），坏数据回默认 |
| `hyperplayer:hrtf-active-dataset` | 活动 HRTF 数据集 id | `setHrtfDataset` 写入 / `restoreHrtfDataset` 读取（跨重启自动恢复锚点） |
| IndexedDB `hyperplayer-hrtf` / `datasets` | 网格本体（结构化克隆） | key = 日期戳 id（字典序=时间序） |
...
HRTF_ACTIVE_DATASET_KEY: string                             // 'hyperplayer:hrtf-active-dataset'
```

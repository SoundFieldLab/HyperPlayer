# HyperPlayer v2 音效引擎深读分析（Task A 交付）

> 分析对象：`HyperPlayer/src/services/audio-effects-v2/`（v2 本地增强版音效引擎）
...
> `HyperPlayer/test/audioEffects.test.ts`、`HyperPlayer/test/compensation.test.ts`。
> 结论对应 v3 目标契约：`HyperSoundEngine-v1/src/dsp/API_SPEC.md` 与 `research/docs/音频算法设计文档.md`。
...
- 私有常量：`SETTINGS_KEY='hyperplayer:audio-effects-settings'`、`MY_SCENES_KEY='hyperplayer:my-scenes'`、
...
| 键 | 归属 | 内容 |
|---|---|---|
| `hyperplayer:audio-effects-settings` | 引擎 | AudioEffectsSettings JSON（深合并加载，旧字段自动补默认） |
| `hyperplayer:my-scenes` | 引擎 | SceneSnapshot[]（上限 8） |
| `hyperplayer:audio-engine-version` | audioEngineVersion.ts | 'v1' 或 'v2'（默认 v1） |
| `hyperplayer:compensation-cache` | compensationService | 补偿设计缓存（上限 60，LRU） |
| `hyperplayer:lufs-cache` | loudnessNormalization | 按 trackKey 的 LUFS 缓存（上限 300，LRU） |
| `hyperplayer:eq-presets` | MixingStudioV2 | EQ 预设（上限 8） |
| `hyperplayer:loudness-comp-hinted` | App.tsx | 低音量提示一次性标记 |
| `hyperplayer:service-3004-toasted` / `hyperplayer:service-3003-toasted` | App.tsx | 服务健康 toast 一次性标记 |
---
...
- 保存：`saveAsMyScene(name)`，id=`my-<时间戳>`，上限 **8**，空名/空白名拒绝；写入 localStorage `hyperplayer:my-scenes`。
...
- 双层缓存：内存 Map + localStorage `hyperplayer:compensation-cache`（上限 60，按时间 LRU 淘汰）。
...
- 缓存：内存 + localStorage `hyperplayer:lufs-cache`（上限 300，LRU），按 trackKey。
...
| `src/services/audioEngineVersion.ts` | `getAudioEngineVersion()`/`setAudioEngineVersion()`，localStorage `hyperplayer:audio-engine-version`，默认 v1 |
...
参考 `HyperPlayer/test/audioEffects.test.ts`（131 行）与 `test/compensation.test.ts`（469 行）：

...
type LegalStringKey =
  ...
  | 's17p2' | 's17p3' | 's17p4' | 'footer1' | 'footer2'
...
const LEGAL_ZH: Record<LegalStringKey, string> = {
  ...
  s5p4: "4. 本软件提供的音频离线导出（MP3）功能仅用于您对允许试听的内容进行学习、研究或音效调试，导出的文件不得用于任何侵权、商业或公开传播用途。导出前页面会明确提示上述限制；如您仍将导出的音频文件上传、传播、分发或用于任何商业用途，该等行为系您本人的操作与选择，所产生的全部法律责任由您自行承担，与开发者无关。",
  s5p5: "5. 为提供固定交叉淡化（Fixed Crossfade）、无缝衔接（含专辑融合）与 MV 音画对齐等功能，本软件会将播放中的音频临时下载到本机缓存目录（analysis-cache，单曲上限约 100MB、总容量上限 2GB）用于本地离线算法处理；该缓存仅用于本地处理，本软件不向您或任何第三方提供任何形式的下载、导出、转存或重分发功能，并会按 30 天未使用、超过容量上限或您主动\"清除缓存\"等规则自动删除。请勿自行长期保存、转存或再分发通过本软件试听的任何内容，尊重版权，支持正版。",
  s6h1: "六、免责声明",
  ...
  s8h1: "八、隐私与数据说明",
  s8p1: "本软件没有任何自建服务器，也不会向任何平台、第三方机构或个人传输、披露或出售您的数据；软件仅与各官方平台建立必要连接以提供您所使用的功能，除此之外不进行任何其他连接。您的登录凭证等本地数据仅保存在您的本地设备（注册表或应用数据目录），数据以明文形式存储，不会上传至任何服务器。",
  ...
  s8li2: "固定交叉淡化、无缝衔接与 MV 音画对齐所需的音频分析缓存（analysis-cache 目录）仅保存在本机，用于本地离线算法处理，并会按 30 天未使用、超过容量上限或您主动\"清除缓存\"自动删除；",

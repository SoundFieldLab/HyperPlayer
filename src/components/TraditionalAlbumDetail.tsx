                      ...
                      {songs.map((song, index) => {
                        ...
                        return (
                          <div key={`${songKey(song)}:${index}`} className={`grid grid-cols-[42px_minmax(0,1fr)_minmax(110px,.7fr)_56px_36px] items-center gap-3 px-4 py-2.5 transition ${active ? (isDark ? 'bg-white/10' : 'bg-pink-50') : isDark ? 'hover:bg-white/[.055]' : 'hover:bg-slate-50'}`}>
                            ...
                            <button type="button" onClick={() => { const artist = song.artists?.[0]; const artistId = artist?.mid || artist?.id; if (artistId) onOpenArtist?.(String(artistId), song.platform || platform) }} className={`hidden truncate text-left text-xs sm:block ${muted}`}>{song.artists?.map(artist => artist.name).join(' / ') || '未知歌手'}</button>

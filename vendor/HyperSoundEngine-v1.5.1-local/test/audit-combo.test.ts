 ...
 *
 * 审计维度（对照主任务书）：
 *  1. 12 个场景预设逐个应用后链路健康：白噪声 FFT 频响有界（±24dB 内）、
...
function runEngine(p: HyperSoundEngineParams, L: Float32Array, R: Float32Array, engine?: HyperSoundEngine): { oL: Float32Array; oR: Float32Array; mono: Float32Array; engine: HyperSoundEngine } {
...
function getScene(id: string): HyperSoundEngineParams {
...
function assertApplySafe(p: HyperSoundEngineParams, label: string): void {
...
// ===========================================================================
// 1) 12 个场景预设逐个应用后链路健康（白噪声 + FFT 频响有界）
// ===========================================================================
...
describe('B. 场景热切换 A→B→A（参数热切换，状态不重置）', () => {
  it('12 场景依次切换后回到 pop：无 NaN、峰值<1.5、块边界无跳变爆音', () => {
    const N = FS * 2 // 2s
    const L = new Float32Array(N)
    const R = new Float32Array(N)
    for (let i = 0; i < N; i++) {
...
describe('C. 分享串（ShareCodec）编解码与防注入', () => {
  it('12 个场景快照 encode→decode 往返参数一致', () => {
    for (const sc of SCENE_PRESETS) {
...
describe('E. EQ Q 补偿 + LoudnessComp 组合', () => {
  ...
  function comboParams(lcMode: 'preset' | 'auto', lcPreset: string): HyperSoundEngineParams {
    const p = createDefaultParams(FS)
    p.eq.enabled = true
...
describe('F. 场景处理后静音衰减（无自激/DC 累计）', () => {
  it('12 场景处理 1s 噪声后切静音，2s 内输出峰值 <1e-3', () => {
    const n1 = FS

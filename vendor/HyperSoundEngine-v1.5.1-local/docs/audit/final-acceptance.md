...
> **历史范围**：本文是早期 TS 主链验收快照，不代表 1.5.1 的 Rust 支线、Phase 0–5、真实设备、真实 SOFA、Firefox 或物理 multichannel 验收。当前状态以 [`phase-status.md`](phase-status.md) 为准。
...
1. **Convolver 块长 > 分区长（B>L）湿路损坏** → 已修复：
   ...
   - 新增正式回归测试 `test/convolver-blocklen.test.ts`（B=128/512/1024/4096 对拍双精度直接卷积，
     maxErr ≤ 3.4e-6、零 NaN、零丢块）；HyperSoundEngineHost script 兜底 4096 块长 + 卷积混响组合已安全。

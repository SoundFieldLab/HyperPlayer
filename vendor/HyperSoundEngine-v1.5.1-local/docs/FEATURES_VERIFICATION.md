# HyperSoundEngine 音频引擎 —— 功能核验报告

> 目的：① 列出本模块实现的功能与代码位置；② 核验代码与效果的正常性（测试证据）；
> ③ 统计 MIT/LGPL 库使用情况；④ 二次调研 GitHub 上的 MIT/LGPL 候选库（避免造轮子）。
> 最终验证：**32 个测试文件 / 331 用例全部通过（含 UI 冒烟 9 项），tsc 0 错误**（0.2.0 时点实测）。
> **状态（2026-08-31）**：本文为 0.2.0 时点功能核验存档；当前规模与门禁以 [`audit/phase-status.md`](audit/phase-status.md) 和 `CHANGELOG.md` 为准。
...
| 候选库 | 许可 | 对应引擎模块 | 评估结论 |
|---|---|---|---|
| `@domchristie/needles` | MIT | LufsMeter（LUFS） | **不替换**：基于 WebAudio 节点（需 AudioContext），引擎是纯 TS 双路径引擎；可作 UI 侧附加参考 |
| `pitchfinder` | **GPLv3** | PitchYin | **不采用**：GPL 传染，引擎自研 YIN 正确 |
...
| ffmpeg.wasm | LGPL | 解码/转码（非本模块范围） | 可选：HyperSoundEngine 侧如需解码增强可评估（未修改链接） |
| tunajs（Web Audio 效果集） | MIT | 效果器参考 | 基于 WebAudio 节点，与引擎纯 DSP 设计不匹配；仅概念参考 |

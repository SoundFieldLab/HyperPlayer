...
describe('ambisonics：decodeFoaToSpeakers（SH→虚拟扬声器解码）', () => {
  it('解码矩阵：单方向声源在对应扬声器增益最大（45° 源 → 45° 扬声器）', () => {
    ...
    const g = new Float64Array(azs.length)
    decodeFoaToSpeakers(encodeSource(45, 0, 1), azs, g)
    ...
    const g2 = new Float64Array(azs.length)
    decodeFoaToSpeakers(encodeSource(0, 0, 1), azs, g2)
    ...
    const g3 = new Float64Array(azs.length)
    decodeFoaToSpeakers([0, 0, 0, 1] as FoaSignal, azs, g3)
    expect(Array.from(g3).every((x) => x === 0)).toBe(true)
  })
...
describe('ambisonics：stereoToFoa（环境提取，块 RMS）', () => {
  ...
  it('纯同相输入 (L=R)：side≈0（Y=0）、W=√2·rms', () => {
    const s = sinBuf(375)
    const f = new Float64Array(4)
    stereoToFoa(s, s, 512, f)
    expect(f[2]).toBeCloseTo(0, 10) // side=(s-s)/2=0 → Y≈0
    expect(f[0]).toBeCloseTo(1, 8) // W = √2·rms(s) = 1
    expect(f[0]).toBeGreaterThan(f[2])
  })

  it('只读取 frameCount 指定的有效帧，非法帧数写零', () => {
    const l = new Float32Array(1024)
    const r = new Float32Array(1024)
    l.fill(1, 128)
    r.fill(1, 128)
    const f128 = new Float64Array(4)
    const f1024 = new Float64Array(4)
    stereoToFoa(l, r, 128, f128)
    stereoToFoa(l, r, 1024, f1024)
    expect(f128[0]).not.toBeCloseTo(f1024[0], 8)
    const silent = new Float64Array([1, 1, 1, 1])
    stereoToFoa(new Float32Array(0), new Float32Array(0), 256, silent)
    expect(Array.from(silent)).toEqual([0, 0, 0, 0])
    silent.fill(1)
    stereoToFoa(l, r, 0, silent)
    expect(Array.from(silent)).toEqual([0, 0, 0, 0])
  })
})
...
describe('ambisonics：foaAmbienceGains（FOA 动态增益调制——stereoToFoa + decode 一步）', () => {
  ...
  it('纯同相输入 (L=R) → W 大 → 4 增益近似相等（对称布局对 W 增益 = 1/√2·W 相同）', () => {
    const s = sinBuf(375)
    const foa = new Float64Array(4)
    const g = new Float64Array(4)
    expect(foaAmbienceGains(s, s, 256, foa, g)).toBe(g)
  ...
  it('纯反相输入 (L=-R) → Y 大 → 增益正负交替（45°+、135°+、225°−、315°−）', () => {
    ...
    const foa = new Float64Array(4)
    const g = new Float64Array(4)
    foaAmbienceGains(s, r, 256, foa, g)
  ...
  it('静音输入 → 全 0；空缓冲/非法块长 → 回退静音', () => {
    ...
    foaAmbienceGains(s, s, 256, foa, out)
    expect(Array.from(out)).toEqual([0, 0, 0, 0])
    foaAmbienceGains(new Float32Array(0), new Float32Array(0), 256, foa, out)
    expect(Array.from(out)).toEqual([0, 0, 0, 0])
    foaAmbienceGains(s, s, 0, foa, out)
    expect(Array.from(out)).toEqual([0, 0, 0, 0])
  })
  ...
  it('确定性：同输入两次调用逐位一致；块长鲁棒（整体 RMS 与切片无关，stereoToFoa 语义透传）', () => {
    ...
    const foa = new Float64Array(4)
    foaAmbienceGains(l, r, 128, foa, a)
    foaAmbienceGains(l, r, 128, foa, b)
    expect(a).toEqual(b) // 确定性（无随机源）
  })
})

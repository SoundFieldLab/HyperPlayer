 ...
 *
 * 集成形态：引擎主链内联级（Limiter 之后），进程内同步调用 TsConvolverBackend，
 * 非独立 worklet 节点。参数随 HyperSoundEngineParams 快照整体下发
 * （`spatial` 段，默认 mode:'off'=逐位直通），经 ShareCodec 分享串编解码。
 *
 * 渲染实现为纯 TS 参考后端（Rust hrtf-core 落地前的对拍 ground truth）：
 * 解析 HRTF（球头模型）→ 每虚拟扬声器双耳分区卷积 → 房间模拟（镜像源+FDN）→ 干湿混合。
 * 角度单位：度；距离单位：米。
 */
...
/** 空间音频完整参数快照（Rust 服务/WASM 后端契约用全量模型；引擎参数模型采用其子集 SpatialSettings） */

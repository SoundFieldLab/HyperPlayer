 ...
 *  - 这里只负责把 HyperSoundEngineHost 接进 WaveForge 的音频图、持久化参数、UI 桥、
 *    SoundTouch 前置链、离线导出等 WaveForge 业务逻辑。
 *
 * 使用方法：WaveForge 项目把本文件复制到其源码目录，并保证
 * `@soundtouchjs/audio-worklet` 依赖已安装。
 */
...
function persistParams(p: HyperSoundEngineParams): void {
...
function restoreParams(sampleRate: number): HyperSoundEngineParams {
  const base = createDefaultParams(sampleRate)
  try {
    const raw = localStorage.getItem(PARAMS_KEY)
    if (!raw) return base
    const saved = JSON.parse(raw) as DeepPartial<HyperSoundEngineParams>
...
function pitchActive(p: HyperSoundEngineParams | null): boolean {
...
function ensureBridge(fs: number): HyperSoundEngineUiBridge {
  ...
  if (wrappedBridge === null || bridgedEngine !== host.engine) {
    ...
    if (!currentParams) currentParams = restoreParams(fs)
    // createHyperSoundEngineUiBridge 构造时会重置为默认参数，这里立即同步为当前快照
...
export function setSystemVolume(volumePercent: number): void {
  ...
  const next = JSON.parse(JSON.stringify(currentParams)) as HyperSoundEngineParams

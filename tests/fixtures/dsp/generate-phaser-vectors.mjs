...
if (!checkout) throw new Error('usage: node generate-phaser-vectors.mjs <hse-v1.5.1-checkout>')
...
const { PhaserEffect } = await import(pathToFileURL(resolve(root, 'dist/index.js')))
...
function writeVector(spec) {
  const effect = new PhaserEffect(spec.sampleRate)
...
const cases = [
  { case: 'case1', sampleRate: 44100, blockSize: 480, frames: 6000, params: { enabled: true, rateHz: 1.5, depth: 0.8, feedback: 0.5, mix: 0.5, stages: 6 }, inputLeft: sineSum(6000, 44100, [{ frequency: 600, amplitude: 0.6, phase: 0 }, { frequency: 2400, amplitude: 0.3, phase: Math.PI / 4 }]), inputRight: lcgNoise(6000, 86004, 0.5), notes: '采用 HSE 官方 mod-effects.case4 的 Phaser 参数与输入，独立排除 Tremolo。' },
  { case: 'case2', sampleRate: 48000, blockSize: 257, frames: 2057, params: { enabled: true, rateHz: 20, depth: 1, feedback: 0.98, mix: 1, stages: 7.5 }, inputLeft: lcgNoise(2057, 97002, 0.65), inputRight: sineSum(2057, 48000, [{ frequency: 997, amplitude: 0.55, phase: Math.PI / 6 }]), notes: '参数上界与 stages=7.5 向正无穷舍入到 8，包含 1 帧尾块。' },
  { case: 'case3', sampleRate: 8000, blockSize: 31, frames: 1003, params: { enabled: true, rateHz: 0.01, depth: 0, feedback: 0, mix: 0, stages: 2.49 }, inputLeft: sineSum(1003, 8000, [{ frequency: 211, amplitude: 0.7, phase: 0 }]), inputRight: lcgNoise(1003, 97003, 0.45), notes: '参数下界、stages=2.49 舍入到 2、mix=0 音频逐位恒等但状态继续推进。' },
  { case: 'case4', sampleRate: 96000, blockSize: 511, frames: 4099, params: { enabled: true, rateHz: 3.25, depth: 0.35, feedback: 0.72, mix: 0.63, stages: 2.5 }, inputLeft: lcgNoise(4099, 97004, 0.72), inputRight: sineSum(4099, 96000, [{ frequency: 173, amplitude: 0.4, phase: Math.PI / 5 }, { frequency: 6400, amplitude: 0.22, phase: Math.PI / 2 }]), notes: '96 kHz、stages=2.5 舍入到 3、反馈与 11 帧尾块。' },
]
